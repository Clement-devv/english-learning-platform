// src/pages/sub-admin/login-themes/CorporateSlateTheme.jsx
// Same visual design as admin CorporateSlateTheme — uses sub-admin auth hook.
...
export default function SubAdminCorporateSlateTheme() {
  const {
    ...
    <>
      ...
      <div className="cs-root" style={branding.fontFamily ? { fontFamily: `'${branding.fontFamily}', 'Plus Jakarta Sans', sans-serif` } : {}}>
            ...
            <svg className="cs-org-svg" viewBox="0 0 100 82" fill="none">
              {edges.map(([a, b], i) => (
                <line key={i}
                  ...
                  strokeDasharray="2 2" className={`cs-edge cs-e${i}`} />
              ))}
              {edges.map(([a, b], i) => (
                <circle key={`f${i}`} r="0.9" fill="#818cf8" className={`cs-flow cs-f${i % 3}`} opacity="0.7">
              ...
              {nodes.map((n, i) => (
                <g key={i} transform={`translate(${n.x}, ${n.y})`}>
                  ...
                  <text x="0" y="1.2" textAnchor="middle" fontSize="2.6" fill={n.accent} fontWeight="600" fontFamily="inherit">
                    {n.label}
                  </text>
                </g>
              ))}
        ...
        <div className={`cs-right${mounted ? ' cs-in' : ''}`}>
          {view === 'forgot' ? (
            <div className="cs-card">
              <div className="cs-card-head">
                ...
                <div>
                  ...
                  <p className="cs-card-sub">Enter your sub-admin email address</p>
                </div>
              </div>
            ...
            <div className="cs-card">
              <div className="cs-card-head">
                ...
                <div>
                  <p className="cs-center-tag">{centerName}</p>
                  <h1 className="cs-card-title">Sub-Admin Portal</h1>
          ...
          <p className="cs-ver">{centerName} Sub-Admin · {new Date().getFullYear()}</p>
        </div>
      </div>
    </>
  );
...
.cs-root { min-height:100vh; display:flex; font-family:'Plus Jakarta Sans','Segoe UI',sans-serif; background:#0f172a; overflow:hidden; position:relative; }
...
.cs-grid { position:absolute; inset:0; pointer-events:none; z-index:1; background-image: linear-gradient(rgba(129,140,248,.018) 1px, transparent 1px), linear-gradient(90deg, rgba(129,140,248,.018) 1px, transparent 1px); background-size:52px 52px; }
...
.cs-dt0{width:2px;height:2px;background:#818cf8;} .cs-dt1{width:1.5px;height:1.5px;background:#38bdf8;} .cs-dt2{width:2px;height:2px;background:#34d399;} .cs-dt3{width:1.5px;height:1.5px;background:#f59e0b;}
@keyframes cs-ptcl { 0%,100%{opacity:0;transform:translateY(0)} 50%{opacity:.45;transform:translateY(-16px)} }
.cs-left { flex:1; min-height:100vh; position:relative; z-index:2; background:linear-gradient(155deg,#0f172a 0%,#131f38 55%,#0e1a30 100%); border-right:1px solid rgba(129,140,248,.06); display:flex; flex-direction:column; align-items:center; justify-content:center; padding:56px 36px; opacity:0; transform:translateX(-24px); transition:opacity .65s ease, transform .65s ease; }
...
.cs-n0{animation-delay:.1s;} .cs-n1{animation-delay:.22s;} .cs-n2{animation-delay:.3s;} .cs-n3{animation-delay:.38s;} .cs-n4{animation-delay:.5s;} .cs-n5{animation-delay:.58s;} .cs-n6{animation-delay:.66s;}
@keyframes cs-npop { from{opacity:0;transform:scale(.5)} to{opacity:1;transform:scale(1)} }
.cs-edge { animation:cs-efade .8s ease both; }
.cs-e0{animation-delay:.4s;} .cs-e1{animation-delay:.5s;} .cs-e2{animation-delay:.6s;} .cs-e3{animation-delay:.7s;} .cs-e4{animation-delay:.8s;} .cs-e5{animation-delay:.9s;}
@keyframes cs-efade { from{opacity:0} to{opacity:1} }
...
.cs-right { width:460px; min-height:100vh; flex-shrink:0; position:relative; z-index:2; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:48px 40px; opacity:0; transform:translateX(24px); transition:opacity .65s ease .12s, transform .65s ease .12s; }
.cs-right.cs-in { opacity:1; transform:translateX(0); }
.cs-card { width:100%; max-width:390px; background:linear-gradient(155deg,#131a2e,#0e1524); border:1px solid rgba(129,140,248,.12); border-radius:20px; overflow:hidden; box-shadow:0 24px 80px rgba(0,0,0,.6), 0 0 0 1px rgba(129,140,248,.04) inset; animation:cs-rise .6s cubic-bezier(.16,1,.3,1) both; position:relative; }
...
.cs-icon-box { width:48px; height:48px; border-radius:13px; flex-shrink:0; background:linear-gradient(135deg,#131e38,#1a2a4a); border:1px solid rgba(129,140,248,.2); display:flex; align-items:center; justify-content:center; box-shadow:0 4px 16px rgba(129,140,248,.14); animation:cs-iglow 3s ease-in-out infinite; }
...
.cs-err { display:flex; align-items:center; gap:9px; background:rgba(239,68,68,.07); border:1px solid rgba(239,68,68,.18); border-radius:10px; padding:10px 14px; margin:12px 24px 0; font-size:12.5px; color:#f87171; font-weight:500; animation:cs-shake .4s ease both; }
...
.cs-inp-wrap { display:flex; align-items:center; gap:9px; height:48px; padding:0 13px; background:rgba(255,255,255,.022); border:1px solid rgba(255,255,255,.07); border-radius:12px; transition:border-color .2s, box-shadow .2s; }
...
.cs-btn { width:100%; height:50px; background:linear-gradient(135deg,#818cf8,#38bdf8); color:white; border:none; border-radius:12px; font-size:14px; font-weight:700; cursor:pointer; font-family:inherit; margin-top:4px; box-shadow:0 6px 20px rgba(129,140,248,.25); transition:opacity .2s, transform .15s, box-shadow .2s; }
...
.cs-warn { display:flex; align-items:flex-start; gap:7px; background:rgba(120,53,15,.1); border:1px solid rgba(120,53,15,.22); border-radius:0 0 20px 20px; padding:10px 16px; margin:12px 0 0; font-size:10.5px; color:#92400e; line-height:1.5; }
...
@media (max-width:920px) { .cs-root { flex-direction:column; } .cs-left { min-height:auto; padding:40px 24px 28px; } .cs-org-wrap { width:280px; height:220px; } .cs-right { width:100%; min-height:auto; padding:28px 20px 44px; } }
@media (max-width:480px) { .cs-org-wrap { display:none; } .cs-logo-bar { position:relative; top:auto; left:auto; margin-bottom:10px; } }
`;
