...
import {
  ...
  ChevronUp, Check, LayoutGrid, Maximize2,
} from "lucide-react";
...
export default function VideoCall({
  ...
  const [viewMode,        setViewMode]        = useState('pip'); // 'pip' | 'split'
  ...
  }, [joined, localVideoTrack, mode, viewMode]);
  ...
  }, [remoteUsers, joined, mode, viewMode, resumeAudio]);
  ...
  if (mode === "video") {
    const cssAnimations = (
      <style>{`
        ...
        /* Remote: full frame visible, no zoom crop */
        .pip-remote video { object-fit: contain !important; background: #111827; }
        /* Self-view PiP: cover fills the small box cleanly */
        .pip-local video  { object-fit: cover !important; }
      `}</style>
    );

    const floatingEmojiOverlay = floatingEmojis.map(fe => (
        ...
        {micError && !localAudioTrack && (
          <div className="bg-red-950/80 border-t border-red-800/50 px-4 py-2 flex items-center justify-between gap-3 flex-shrink-0 backdrop-blur-sm">
            <div className="flex items-center gap-2 text-red-300 text-xs font-medium">
              <MicOff className="w-3.5 h-3.5 flex-shrink-0" />{micError}
            </div>
            <button onClick={retryMic} className="px-3 py-1.5 bg-red-700 hover:bg-red-600 active:scale-95 text-white text-xs font-semibold rounded-lg transition-all flex-shrink-0">
              Retry Mic
            </button>
          </div>
        )}
        {audioBlocked && (
          <div className="bg-amber-950/80 border-t border-amber-800/50 px-4 py-2 flex items-center justify-between gap-3 flex-shrink-0 backdrop-blur-sm">
            <div className="flex items-center gap-2 text-amber-300 text-xs font-medium">
              <MicOff className="w-3.5 h-3.5 flex-shrink-0" />Audio blocked — click to hear the other person
            </div>
    ...
    const controlBar = joined && (
      <div className="flex-shrink-0 px-4 pb-4 pt-2"
        style={{ background: "linear-gradient(to top, rgba(0,0,0,0.55) 0%, transparent 100%)" }}>
        <div className="flex items-end justify-center gap-2 relative flex-wrap">
          ...
          <div className="relative flex flex-col items-center">
            {showMicPicker && (
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50 rounded-2xl border border-white/10 overflow-hidden"
                ...
                <div className="px-3 py-2 border-b border-white/10 text-[11px] font-semibold text-white/40 uppercase tracking-wider">Microphone</div>
                {micDevices.length === 0
            ...
            <button onClick={() => { setShowMicPicker(v => !v); loadMicDevices(); }}
              className="mb-0.5 px-1.5 py-0.5 rounded-md text-white/40 hover:text-white/80 hover:bg-white/10 transition-all" title="Choose microphone">
              <ChevronUp className="w-3 h-3" />
            </button>
            <CtrlBtn onClick={localAudioTrack ? toggleMic : retryMic}
              label={!localAudioTrack ? "No Mic" : micOn ? "Mute" : "Unmuted"}
              active={micOn && !!localAudioTrack} danger={!micOn || !localAudioTrack}
          ...
          {/* ── Camera ── */}
          <CtrlBtn onClick={toggleCamera} label={camOn ? "Camera" : "No Cam"} active={camOn} danger={!camOn}>
            {camOn ? <VideoIcon className="w-5 h-5" /> : <VideoOff className="w-5 h-5" />}
          </CtrlBtn>

          {/* ── Screen share ── */}
          <CtrlBtn onClick={toggleScreenShare} label={isScreenSharing ? "Stop Share" : "Share"} active={!isScreenSharing} accent={isScreenSharing ? "blue" : null}>
          ...
          {vbCompatible && vbProcessorRef.current && (
            <div className="relative">
              {BgPanel}
              <CtrlBtn onClick={() => { setShowBgPanel(v => !v); setShowEmojiPicker(false); setShowChat(false); setShowMicPicker(false); }}
                label="Background" active={!showBgPanel && activeBg === "none"} accent={activeBg !== "none" || showBgPanel ? "purple" : null}>
          ...
          {/* ── Chat ── */}
          <div className="relative">
            <CtrlBtn onClick={handleToggleChat} label="Chat" active={!showChat} accent={showChat ? "indigo" : null}>
          ...
          <div className="relative">
            ...
            <CtrlBtn onClick={() => { setShowEmojiPicker(v => !v); setShowBgPanel(false); setShowMicPicker(false); }}
              label="React" active={!showEmojiPicker} accent={showEmojiPicker ? "yellow" : null}>
          ...
          {noiseCancelSupported && (
            <CtrlBtn onClick={toggleNoiseCancel} label={noiseCancel ? "Noise Off" : "Noise On"}
              active={!noiseCancel} accent={noiseCancel ? "teal" : null}
            ...
            <div className="relative">
              {isRecording && (
                <div className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap bg-black/80 text-white text-[10px] font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5 backdrop-blur-sm">
                  <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />{formatRecTime(recSeconds)}
                </div>
              )}
              <CtrlBtn onClick={isRecording ? stopRecording : startRecording} disabled={uploadingRecording}
                label={uploadingRecording ? "Saving…" : isRecording ? "Stop Rec" : "Record"}
                active={!isRecording && !uploadingRecording} accent={isRecording ? "red" : null} pulse={isRecording}>
                {uploadingRecording ? <Loader className="w-5 h-5 animate-spin" /> : isRecording ? <Square className="w-5 h-5 fill-current" /> : <Circle className="w-5 h-5" />}
              </CtrlBtn>
            </div>
          )}

          {/* ── Layout toggle ── */}
          <CtrlBtn onClick={() => setViewMode(v => v === 'pip' ? 'split' : 'pip')}
            label={viewMode === 'pip' ? "Split View" : "Focus View"} active>
            {viewMode === 'pip' ? <LayoutGrid className="w-5 h-5" /> : <Maximize2 className="w-5 h-5" />}
          </CtrlBtn>
    ...
    // ── PiP layout (default): remote fills screen, local is bottom-right overlay ──
    if (viewMode === 'pip') {
      return (
        <div className="h-full flex flex-col bg-gray-950 relative overflow-hidden">
          ...
          <div className="flex-1 relative min-h-0 bg-gray-900">
            ...
            {joined && remoteUsers.length > 0 ? remoteUsers.map(user => (
              <div key={user.uid} className="absolute inset-0 flex items-center justify-center p-3 bg-gray-950">
                <div className="relative w-full h-full rounded-2xl overflow-hidden ring-1 ring-white/10 shadow-2xl">
                  <div ref={el => { remoteContainerRef.current[user.uid] = el; }} className="pip-remote w-full h-full" />
                  {!user.videoTrack && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-800">
                      <div className="w-24 h-24 rounded-full bg-white/10 flex items-center justify-center mb-4">
                        <Users className="w-12 h-12 text-white/40" />
                      </div>
                      <p className="text-white/40 font-medium">Camera off</p>
                    </div>
                  )}
                  <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm px-2.5 py-1 rounded-lg flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                    <span className="text-sm font-medium text-white/90">Connected</span>
                  </div>
                  <div className="absolute top-4 left-4 bg-red-600/90 backdrop-blur-sm text-white text-xs font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1.5">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                {joined ? (
                  <>
                    ...
                    <p className="font-semibold text-white/30 text-xl">Waiting for others…</p>
                    <p className="text-sm text-white/20 mt-1">They'll appear here when they join</p>
                  </>
                ) : loading ? (
                  <>
                    <Loader className="w-16 h-16 text-purple-500 animate-spin mb-3" />
                    <p className="font-bold text-purple-600 text-xl">Connecting…</p>
                    <p className="text-sm text-purple-400 mt-1">Starting camera and microphone</p>
                  </>
                ) : (
                  <>
                    <VideoIcon className="w-16 h-16 text-purple-400 mb-3" />
                    <p className="font-bold text-purple-600 text-xl">Ready to join</p>
                  </>
                )}
              </div>
            )}
              ...
              <div className="absolute bottom-4 right-4 z-20 rounded-2xl overflow-hidden shadow-2xl ring-2 ring-white/20"
                ...
                {camError && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900/90 p-2 text-center">
                    ...
                    <p className="text-white/70 text-[10px] leading-tight mb-1.5">{camError}</p>
                    <button onClick={retryCamera} className="px-2.5 py-1 bg-purple-600 hover:bg-purple-500 text-white text-[10px] font-semibold rounded-lg transition-colors">Retry</button>
                  </div>
                )}
                <div className="absolute bottom-1.5 left-2">
                  <span className="text-[10px] font-medium text-white/80 bg-black/50 px-1.5 py-0.5 rounded-md backdrop-blur-sm">{userName}</span>
                </div>
                <div className="absolute top-1.5 right-1.5 flex gap-1">
                  {!micOn          && <div className="bg-red-500 p-0.5 rounded-full"><MicOff   className="w-2.5 h-2.5 text-white" /></div>}
                  {!camOn          && <div className="bg-red-500 p-0.5 rounded-full"><VideoOff className="w-2.5 h-2.5 text-white" /></div>}
                  {isScreenSharing && <div className="bg-blue-500 p-0.5 rounded-full"><Monitor  className="w-2.5 h-2.5 text-white" /></div>}
                </div>
              </div>
            )}
    ...
    // ── Split layout: two equal panels side by side ──
    return (
      <div className="h-full flex flex-col bg-gray-950 relative overflow-hidden">
        {cssAnimations}
        {floatingEmojiOverlay}
